    ...
    public class RestierController : ODataController
    {
        ...
        public async Task<IActionResult> Get(CancellationToken cancellationToken)
        {
            ...
            else
            {
                if (queryable == null)
                {
                    return this.NotFound(Resources.ResourceNotFound);
                }
        ...
        public async Task<IActionResult> Post(EdmEntityObject edmEntityObject, CancellationToken cancellationToken)
        {
            ...
            var changeSetProperty = this.HttpContext.GetChangeSet();
...
#pragma warning restore CA1062 // Validate public arguments
        ...
        private IActionResult CreateQueryResponse(IQueryable query, IEdmType edmType, ETag etag)
        {
            ...
            if (typeReference.IsComplex())
            {
                ...
                response = this.Ok(complexResult);
            }
            ...
            if (typeReference.IsCollection())
            {
                ...
                {
                    return this.Ok(new NonResourceCollectionResult(query, typeReference));
                }

                return this.Ok(new ResourceSetResult(query, typeReference));
            }
        ...
        {
            var properties = this.HttpContext.ODataFeature();
            if (properties == null)
            {
        ...
        private async Task<IReadOnlyDictionary<string, object>> GetOriginalValues(IEdmEntitySet entitySet)
        {
            ...
            if (this.Request.Headers.TryGetValue("IfMatch", out var ifMatchValues))
            {
            ...
            if (this.Request.Headers.TryGetValue("IfNoneMatch", out var ifNoneMatchValues))
            {
        ...
        private IActionResult CreateCreatedODataResult(object entity) => this.CreateResult(typeof(CreatedODataResult<>), entity);

        private IActionResult CreateUpdatedODataResult(object entity) => this.CreateResult(typeof(UpdatedODataResult<>), entity);
        ...
        {
            var container = this.HttpContext.Request.GetRequestContainer();
            this.api = container.GetRequiredService<ApiBase>();
            this.querySettings = container.GetRequiredService<ODataQuerySettings>();
            this.validationSettings = container.GetRequiredService<ODataValidationSettings>();
            this.operationExecutor = container.GetRequiredService<IOperationExecutor>();
        }
    }
}
