...
namespace Microsoft.Restier.Core.Tests
{
    ...
    public class ServiceCollectionExtensionsTests : IClassFixture<ServiceProviderFixture>
    {
        ...
        public void CanCallAddRestierCoreServices()
        {
            ...
            var result = services.AddRestierCoreServices(typeof(TestApi));

            result.HasService<ApiBase>().Should().BeTrue();
            result.HasService<ApiConfiguration>().Should().BeTrue();
        ...
        {
            Action act = () => default(IServiceCollection).AddRestierCoreServices(Type.GetType("TestValue1314519768", false, false));
            act.Should().Throw<ArgumentNullException>();
        }

        /// <summary>
        /// Cannot call AddRestierCoreServices with null api type.
        /// </summary>
        [Fact]
        public void CannotCallAddRestierCoreServicesWithNullApiType()
        {
            Action act = () => new Mock<IServiceCollection>().Object.AddRestierCoreServices(default(Type));
            act.Should().Throw<ArgumentNullException>();
        }
