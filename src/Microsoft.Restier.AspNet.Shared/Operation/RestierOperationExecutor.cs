// <copyright file="RestierOperationExecutor.cs" company="Microsoft Corporation">
...
#pragma warning disable CA1062 // Validate arguments of public methods. JWS: Ensure.NotNull is there. Spurious warning.
            await this.InvokeAuthorizers(restierOperationContext, cancellationToken).ConfigureAwait(false);
#pragma warning restore CA1062 // Validate arguments of public methods.
            ...
            var model = await restierOperationContext.Api.GetModelAsync(cancellationToken).ConfigureAwait(false);
            ...
            await this.PerformPreEvent(restierOperationContext, cancellationToken).ConfigureAwait(false);

            var result = await InvokeOperation(restierOperationContext.Api, method, parameters, model).ConfigureAwait(false);

            // Invoke preprocessing on the operation execution
            await this.PerformPostEvent(restierOperationContext, cancellationToken).ConfigureAwait(false);
            return result;
        }
        ...
        {
            if (this.operationAuthorizer == null)
            {
                return;
            }

            if (!await this.operationAuthorizer.AuthorizeAsync(context, cancellationToken).ConfigureAwait(false))
            {
        ...
        private async Task PerformPreEvent(OperationContext context, CancellationToken cancellationToken)
        {
            ...
            {
                await this.operationFilter.OnOperationExecutingAsync(context, cancellationToken).ConfigureAwait(false);
            }
        }
        ...
        private async Task PerformPostEvent(OperationContext context, CancellationToken cancellationToken)
        {
            if (this.operationFilter != null)
            {
                await this.operationFilter.OnOperationExecutedAsync(context, cancellationToken).ConfigureAwait(false);
            }
        }
    }
}
