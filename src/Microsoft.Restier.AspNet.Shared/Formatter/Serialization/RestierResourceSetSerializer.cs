// <copyright file="RestierResourceSetSerializer.cs" company="Microsoft Corporation">
...
#pragma warning disable CA1062 // Validate public arguments
                if (this.TryWriteAggregationResult(graph, type, messageWriter, writeContext, collectionResult.EdmType))
            ...
            IEdmTypeReference resourceSetType)
        {
            if (typeof(IEnumerable<DynamicTypeWrapper>).IsAssignableFrom(type))
            {
                ...
                if (elementType.IsEntity())
                {
                    ...
                    this.WriteObjectInline(graph, resourceSetType, writer, writeContext);
                    return true;
                }
            }
